...
\ShowFig{0.6}{ht}{validator}{Validatorのトップ画面}{validator}
\begin{enumerate}
 \item 図\ref{validator}でNu Html Checkerをクリックすると図
			 \ref{nu-checker}が表示されます。
\ShowFig{0.6}{ht}{nu-checker}{Nu Html Checkerの画面}
{nu-checker}
 \item ここで「Check by」のプルダウンメニュで「file upload」を設定し、
       「ファイル選択」でチェックしたいファイルを指定します図\ref{file-upload}。
\ShowFig{0.6}{ht}{file-upload}{ファイルのアップロード}{file-upload}
 \item ここで「check」のボタンを押すとデータが送られてデータの検証
       が行われます(図\ref{result})。
\ShowFig{0.6}{ht}{result}{検証結果の表示画面}
{result}
 \item このページを下のほうに間違いの理由と場所がより具体的に指摘されます。
\end{enumerate}
\iffalse
