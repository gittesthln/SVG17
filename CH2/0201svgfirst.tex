...
色の3原色、赤、緑、青のそれぞれに対し$0\sim255$ の値を割り当てて指定し
       ます。%次のようになっています。
%
なお、\AttribCVal{rgb}{}関数では $0\sim255$の値ではなく \Showattrib{\%}で
...
{\AttribVal{scale}{}}{拡大・縮小}{\AttribVal{scale}{(0.5)},
       ...
       ここでは\AttribVal{translate}{(50,50)}となっているので原点の位置
...
\ShowFig{0.35}{ht}{fick1}{フィックの錯視}{example-fick}
%\label{fick}
...
\ShowFig{0.5}{ht}{muller-lyer}{ミューラー$\cdot$ライヤー錯視}
{example-muller-lyer}
...
\OIIdxMC{ポッケンドルフ}{58}{}として知られる図形で
 す。長方形と直線を使って描いています。
\ShowFig{0.3}{ht}{poggendorf}{ポッケンドルフ錯視}
{poggendorf}
...
という錯視図形です。
\ShowFig{0.5}{ht}{colorinenv}{周りの濃度で見え方が異なる}
{whiteIO0}
