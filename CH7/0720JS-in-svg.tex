...
\begin{itemize}
 ...
 定義されています(\LineR{init-start}{init-end})。\Attrib{onload}のイベントは\SVG がすべて読み込まれた後、呼び出されます。
  \item まず、\Elm{circle}を持つオブジェクトのリストを得ます
       (\Line{getElements})。
   \DOMM{getElementsByTagName}がそのメソッドです。これはオブジェクトのコレ
   クションを返します。コレクション内のオブジェクトの数は\JSM{length}プ
       ロパティで得られます。
...
%\newpage
\ShowFigs{0.35}{ht}{{svg-js-click3-1}{svg-js-click3-2}}
...
く、その位置がSVG内に表示されます。
\ShowFigs{0.35}{ht}{{svg-js-showclickpos-1}{svg-js-showclickpos-2}}
...
\begin{itemize}
 ...
 \item \Event{mouseup}イベントに対してはグローバル変数
       \Event{mousemove}のｓっ歩ｒ関数を取り除いています
       (\Line{reset})。したがって、イベントハンドラ-はドラッグして
       いる間だけ処理が行われます。
 \item \Line{moves}から\Line{moveE}で\Event{mousemove}の処理をしています。
 \item まず、グローバル変数\texttt{inDragging}が\ElmJ{null}でない場合
       (\Line{indragging})はドラッグ中なのでイベントが発生したマウスポイ
       ンタの位置をドラッグ中の円の中心位置に設定しています(\Line{setcx}
       と\Line{setcy})。
 \item なお、\Event{mousemove}イベントはいつもドラッグ中の円で起こって
       いるわけではありません。二つの円が重なった場合、上に表示されてい
       るほうの円で起こっています。イベントを処理する関数が\Elm{svg}で定
       義されているのでイベントが拾えています。
\end{itemize}
...
\begin{itemize}
 \item \Event{mousemove}が割り当てられるのは円上で\Event{mousedown}が起
       きたときだけにします。
 \item \Event{mouseup}のイベント処理中に\Event{mousedown}を処理する関数
       を取り除きます。
...
\begin{itemize}
 \item \Line{svg}でルートのオブジェクトを求め、グローバル変数\texttt{G}に保存し
       ています(\Line{getelm})。
 \item \Line{addEventListener1}と\Line{addEventListener2}で
       それぞれ\Event{mousedown}と\Event{mouseup}を処理する関数を
       \texttt{G}に割り当てています。
 \item \LineR{mdownS}{mdownE}では\Event{mousedown}を処理する関数
       \texttt{mdown}を定義しています。
\begin{itemize}
 \item オブジェクトが\Elm{svg}でない場合(\Line{checkObject})に変数
       \texttt{inDragging}にイベントが起きたオブジェクトを格納します
	  (\Line{saveObject})。演算子\verb+!==+に注意してください。
    \item \Line{changePos}ではドラッグが開始されたオブジェクトを親オブジェ
    ...
    \item グローバル変数\texttt{G}に保存したオブジェクト(\Elm{svg})に
	  \Event{mousemove}を処理する関数を割り当てています
	  (\Line{addEventListener3})。
\end{itemize}
 \item \Event{mousemove}イベントを処理する関数は\Line{move}から始まりま
       す。ドラッグ中のオブジェクトの中心位置を変えた後、イベントが他の
       オブジェクトに伝播しないように\DOMM{stopPropagation}{()}を実行して
       います(\Line{stopProp})。
 \item \Event{mouseup}のイベント処理ではグローバル変数\texttt{G}に割り当
       てられた\Event{mousemove}の処理関数を取り除くための
     \DOMM{removeEventListener}{()}メソッドを呼び出しています
...
{\ElmF{feTurbulance}フィルタのパラメータをスクロールバーで設定する}
...
\begin{enumerate}
 ...
 \item Chrome の DOMパネルをエクスポートボタン(図
       \ref{add-lines-check}の赤丸で囲まれた部分)をクリックすると起
       こることを報告しなさい\footnote{これ以外のブラウザでは要素の上で
       右クリックすると「Edit as HTML」のような選択ができます。}。
...
\ShowFig{0.4}{ht}{svg-js-add-lines-color}{色を変えて直線を引く}
