...
\begin{itemize}
 \item すべてのグローバル変数を初期化\Func{init()}内に移動しています
       (\LineR{GlobalS}{GlobalE})。
 \item \Func{init()}内でロカールな関数を定義しています
			 (\LineR{ClosureS}{ClosureE})。
\begin{itemize}
 \item 関数は無名関数となっています(\Line{ClosureS})。
 ...
 \item \Line{join}の\ElmJ{join}は配列の要素を、引数の文字列をはさんでつ
       ...
       用いるよりも簡単に記述できます。
 \item \Line{callee}ではこの関数の引数のリストが参照できる
			 \ElmJ{arguments}のメンバー\ElmJ{callee}を用いて、自分自身がこの後、
			 呼び出せるように設定しています。
\end{itemize}
