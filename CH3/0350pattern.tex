...
\end{itemize}
\ProbwithFigSol{savigny}{0.5}{ht}{ザビニの錯視}{svg-savigny}
{\OIIdxM{ザヴィニー}\cite[132ページ]{Ninio}とよばれます。両方のパターンを囲む正方形の大
きさは同じなのですがパターンの向きにより異なった方向のほうが長い長方形に
見えます。パターンには\Elm{path}を用いると良いでしょう。\par
この図を描きなさい。}
...
%
\ShowFig{0.4}{ht}{pattern}
   {二つの線形グラデーションを市松模様に並べた例}
{pattern-example}
\SVGListN{二つの線形グラデーションを市松模様に並べた例}
   {patterndef1}{svg-pattern}
\begin{itemize}
 \item \Line{grad1S}から\Line{grad1E}と\Line{grad2S}から\Line{grad2E}で2
       つの線形グラデーションを定義しています。
 \begin{itemize}
  \item \Line{grad1S}から\Line{grad1E}は上から下へ向
	かう線形グラデーションを定義しています。
  \item \Line{grad2S}から\Line{grad2E}は左から右へ向
	かう線形グラデーションを定義しています。
 \end{itemize}
...
%\clearpage
\ProbwithFigSol{hermann-twinkle}{0.4}{ht}{輝くヘルマン格子}{svg-hermann-twinkle}
...
\ProbwithFigSol{morgan-twist}{0.4}{ht}{モーガンのねじれひも}{svg-morgan-twist}
...
\begin{itemize}
 ...
 \item パターンは領域全体を白で塗りつぶされた長方形(\Line{rectW})とその
       上に対角線上に二つ並んだ黒く塗りるつぶされた長方形(\Line{path})か
       らなります。
 \item \LineR{CS}{CE}で外側の円をこのパターンで塗りつぶしています。
 \item その上に小さな円を同じパターンで塗りつぶし(\Line{C2})、さらに
       $(200,200)$ を中心に $90^{\circ}$ 回転(\Line{rotate})したものを描
       いています。
\end{itemize}
...
せた図形を内部に持つ長方形です。この図は\Elm{pattern}に属性
 \AttribO{patternTransform}を用いて作成できます。この図を作成しなさい。
    \ShowFig{0.4}{ht}{pattern-rotate}
